// Server-side MoonAi agent for the web console. Runs a real OpenAI tool-calling
...
async function agentPolicyTool(args: Record<string, unknown>, ctx: ToolContext) {
  const id =
    ...
    process.env.NEXT_PUBLIC_MOONAI_POLICY_OBJECT_ID ||
    null
...
async function recentReceiptsTool(args: Record<string, unknown>, ctx: ToolContext) {
  const agent =
    ...
    process.env.NEXT_PUBLIC_MOONAI_AGENT_ADDRESS ||
    ctx.walletAddress
...
const SYSTEM_PROMPT = `You are moonai, a Sui-native, policy-aware AI treasury assistant.
...
const OPENAI_URL =
  (process.env.MOONAI_LLM_BASE_URL ?? 'https://api.openai.com/v1') + '/chat/completions'
...
export async function runAgent(
  ...
  const apiKey = process.env.OPENAI_API_KEY || process.env.MOONAI_LLM_API_KEY
  if (!apiKey)
    return {
