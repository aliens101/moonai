...
const DERIVATION_DOMAIN = 'moonai-agent:v1:'
...
export function deriveAgentKeypair(ownerAddress: string): Ed25519Keypair {
  const master = process.env.MOONAI_MASTER_SECRET
  if (!master || master.length < 32) {
    throw new Error('MOONAI_MASTER_SECRET is not configured on the server (need ≥32 chars)')
  }
