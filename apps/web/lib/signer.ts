...
//     MOONAI_MASTER_SECRET. Simple, but the master lives in the app process.
...
//     a separate hardened process and point MOONAI_SIGNER_URL at it).
//
...
export function getAgentSigner(): AgentSigner {
  const url = process.env.MOONAI_SIGNER_URL
  return url ? new RemoteSigner(url, process.env.MOONAI_SIGNER_API_KEY) : new LocalDerivedSigner()
}
