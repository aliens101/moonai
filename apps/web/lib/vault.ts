...
import { CLOCK, MOONAI_PKG, ORIGINAL_PKG, SUI_TYPE, VAULT_PKG } from '@/lib/onchain-constants'
import { webSuiClient } from '@/lib/ops'

// LATEST package id (env override wins), re-exported for the execution path.
export const PKG = process.env.MOONAI_PACKAGE_ID ?? MOONAI_PKG
...
export async function readAllowedRecipients(policyId: string): Promise<string[] | null> {
  try {
    const o = await sui.getDynamicFieldObject({
      parentId: policyId,
      name: { type: 'vector<u8>', value: Array.from(new TextEncoder().encode('moonai.recipients')) },
    })
