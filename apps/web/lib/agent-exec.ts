...
// on-chain treasury Vault (moonai::vault). This module signs with the SIGNED-IN
...
function preCheck(amountMist: bigint, coinTypeIn: string): string | null {
  const maxPerTx = BigInt(Math.round(Number(process.env.MOONAI_POLICY_MAX_PER_TX_SUI ?? '1') * 1e9))
...
async function executeSwap(
  ...
  const me = agentAddr
  const slippageBps = Number(process.env.MOONAI_POLICY_MAX_SLIPPAGE_BPS ?? '100')
